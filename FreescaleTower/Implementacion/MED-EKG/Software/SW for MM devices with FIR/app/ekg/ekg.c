...
void EcgDiagnosticModeStartMeasurementReq(void)
{
    ...
    if (EcgFir_DiagnosticModeStartMeasurement() == TRUE)
		{
			g_curr_send_buf[g_send_size++] = ERROR_OK;
		}
		else
...
void EcgDiagnosticModeStopMeasurementReq(void)
{
	...
	#elif (defined ECG_FIR)
	  EcgFir_DiagnosticModeStopMeasurement();
		Ecg_DiagnosticModeStopMeasurement();
	#else
		Ecg_DiagnosticModeStopMeasurement();
	#endif
...
void EcgDiagnosticModeNewDataReadyInd(void)
{
  ...
  if (ActualMeasurement == ECG_MEASUREMENT)
    {
     UINT8 u8DataCounter=MAX_ECG_DATA;
     
     g_curr_send_buf[g_send_size++] = IND;
		 g_curr_send_buf[g_send_size++] = ECG_DIAGNOSTIC_MODE_NEW_DATA_READY;
		 g_curr_send_buf[g_send_size++] = (MAX_ECG_DATA*2)+3;				//ECG data + HR + PacketID
		 ...
		 while (u8DataCounter)
		  {
		   u8DataCounter--;
		   g_curr_send_buf[g_send_size++] = (UINT8)(au16EcgFilteredData[u8DataCounter]>>8); //Upper 8 Bytes
		   g_curr_send_buf[g_send_size++] = (UINT8)(au16EcgFilteredData[u8DataCounter] & 0x00FF); //Lower 8 Bytes 
		  }
     g_curr_send_buf[g_send_size++] = Averaged_HR;
