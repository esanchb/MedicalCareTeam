...
__declspec(vectortable) vectorTableEntryType _vect[256] = {   /* Interrupt vector table */
   ...
   asm_exception_handler,           /*  64 (0x100) Device-specific interrupts */
	...
	IRQ_ISR,           				/*  65 (0x104) IRQ1 Interrupt Handler     */
#else   
...
#ifdef __MCF52221_H__
	IRQ_ISR,                        /*  71 (0x11C) IRQ7 Interrupt Handler     */
#else
...
#ifdef __BRIDGE__
   UART0_ISR,           			/*  77 (0x134) Device-specific interrupts */
#else
...
#if((defined(__MCF52259_H__) || defined(__MCF52221_H__)) && (MAX_TIMER_OBJECTS > 0))
   Timer_ISR,        				/* 119 (0x___) Prog. Interrupt Timer 0    */
#else
   asm_exception_handler,           /* 119 (0x___) Reserved                   */
#endif
#if (defined(USED_PIT1))
   PIT1_ISR,           				/* 120 (0x___) Prog. Interrupt Timer 1    */
#else
   ...
   Timer_ISR,           			/* 171 (0x___) Prog. Interrupt Timer 0    */
   #else
   ...
   #ifdef __MCF52277_H__
   USB_ISR,           				/* 175 (0x___) USB Interrupt Handler      */
   #else
